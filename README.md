...
[Slide Deck Presentation](https://www.youtube.com/watch?v=3mtk6OpgFpk&ab_channel=CallumRowston)
...
|----|----|----|
|Test when user guesses 'crate' and the secret word is 'CRATE'|The word 'CRATE' is printed in green and the game condition user_wins will return True    |✅    |
|Test when user guesses 'crate' and the secret word is 'CRANE'|The letters 'C', 'R', 'A' and 'E' will be printed in green and 'T' will be printed in white in order C-R-A-N-E    |✅  |
|Test when user has remaining guesses|A new line with underscores for the amount of letters in the word is printed below the users most recent guess    |✅   |
|Test when user_wins returns True |Print the user wins and number of guesses and end_menu() will be called to allow the user to play again or exit    |✅   |
|Test when user_loses returns True |Print the user loses and end menu() will be called to allow the user to play again or exit   |✅   |
|Test when user_wins or user_loses returns True and game_is_default returns True    |add_game_stats() will be called and correctly update stats.json    |✅  |
|Test when user accesses stats menu    |Displays cumulative stats from current and previous sessions and return menu    |✅    |
|Test when user accesses rules menu    |Displays rules with correct formatting and return menu    |✅    |
|Test when user selects Quit or Exit to desktop from a menu    |Game prints that it is quitting and the application terminates    |✅   |
|Test when user selects Back To Main Menu from a menu    |Game returns to the main menu    |✅ |
|Test when user accesses game settings menu    |Displays current settings, warning, and second menu to change settings    |✅   |
|Test when user selects Set Word Length menu option and enters 8    |Global variable WORD_LENGTH_SETTING is set to 8    |✅   |
|Test when user selects Set Word Length menu option and enters 9    |RangeError is raised and user prompted to try again    |✅   |
|Test when user selects Set Word Length menu option and enters 'nothing'    |ValueError is raised and user prompted to try again    |✅    |
|Test when user selects Set Word Length menu option and enters 'back'     |The application returns to the game settings menu    |✅   |
|Test when user selects Set Total Guesses menu option and enters 9    |Global variable TOTAL_GUESSES_SETTING is set to 9    |✅  |
|Test when user selects Set Total Guesses menu option and enters 10    |RangeError is raised and user prompted to try again    |✅   |
|Test when user selects Set Total Guesses menu option and enters 'nothing'    |ValueError is raised and user prompted to try again    |✅   |
|Test when user selects Set Total Guesses menu option and enters 'back'     |The application returns to the game settings menu    |✅   |
|Test when Reset To Default Settings menu option is selected    |WORD_LENGTH_SETTING is set to 5 and TOTAL_GUESSES_SETTING is set to 6    |✅  |
|Test when WORD_LENGTH_SETTING is set to 8    |Secret word will be an 8 letter word and the user can only enter 8 letter words    |✅  |
|Test when TOTAL_GUESSES_SETTING is set to 9    |User is allowed 9 guesses before user_loses returns True    |✅ |
|Test when WORD_LENGTH_SETTING is set to 6 and TOTAL_GUESSES_SETTING is set to 7    |Secret word will be a 6 letter word and the user can only enter 6 letter words and user is allowed 7 guesses before user_loses returns True    |✅   |
...
This code conforms to the [PEP 8 Style Guide For Python Code](https://peps.python.org/pep-0008/), by Guido van Rossum, Barry Warsaw and Nick Coghlan.
